...
from .vit_transformer import Transformer
from .vit_utils import load_pretrained_weights, as_tuple
...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'pre_logits'):
            x = self.pre_logits(x)
            x = torch.tanh(x)
        layerwise = [self.norm(x_[:,1:]) for x_ in x]
        layerwise = [x.transpose(2,1).reshape(b, -1, self.gh, self.gw) for x in layerwise]
        layerwise.append(self.norm(x[-1][:,0].reshape(b,-1)))
        return layerwise
